...
const AppSidebar: React.FC = () => {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        ...
        const user = response.data; // Extract data from the response
  ...
  // Filter navItems based on user condition
  const getNavItems = (): NavItem[] => {
    ...
    if (connectedUser.username === "36469820") {
      ...
      return allNavItems.filter(item =>
          item.name === "Dashboard" || item.name === "Optics"
      );
    }
  };
  ...
  useEffect(() => {
    if (!connectedUser) return; // Don't process submenus until user data is loaded
  ...
  }, [pathname, isActive, navItems, connectedUser]);
  ...
  // Hide sidebar on dashboard page (root path)
  if (pathname === '/') {
    return null;
  }

  // Show loading state or nothing while fetching user data
  if (isLoading) {
    return (
        <aside
            className={`
          fixed mt-16 flex flex-col lg:mt-0 top-0 px-5 bg-white dark:bg-gray-900 dark:border-gray-800 text-gray-900 h-screen transition-all duration-300 ease-in-out z-50 
          ...
          >
            <Link href="/">
              {isExpanded || isHovered || isMobileOpen ? (
                  ...
                  <Image
                      src="/images/logo/browser_logo.png"
                      alt="Logo"
                      width={32}
                      height={32}
                  />
  ...
  // Don't render menu if no user data (optional safety check)
  if (!connectedUser) {
    return (
        <aside
            className={`
